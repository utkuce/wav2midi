...
struct Audio {
    data: Vec<f64>, sample_rate : usize, channels : usize,
}
...
fn main() {
    ...
    let audio = Audio { 
        data : reader.samples::<i16>().map(|sample| sample.unwrap() as f64).collect(),
        sample_rate : reader.spec().sample_rate as usize,
        channels : reader.spec().channels as usize
    };

    write_spectrogram_data(&audio, 4096, 512, "rhythm.csv");
    write_spectrogram_data(&audio, audio.sample_rate, 512, "pitch.csv");
}

fn write_spectrogram_data(audio : &Audio, window_size: usize, step_size: usize, file_name: &str) {
    ...
    for some_samples in (&audio.data[..]).chunks(3000) {
        ...
        while stft.contains_enough_to_compute() {
            ...
            assert!(result.is_ok());
            println!("{}/{}", count, 
                (audio.data.len()-(window_size-step_size)) /step_size );
            count +=1;
